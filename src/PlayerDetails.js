...
const getPlayerImageUrl = (faceImageId) => {
  if (!faceImageId || faceImageId === 0)
    return `https://placehold.co/200x200/1e3a5f/0ea5e9?text=?`;
...
const PlayerDetails = () => {
  ...
  const getPlayerSEOConfig = (profileData) => {
    if (!profileData) {
      return {
        ...
        description:
          "View detailed cricket player profile, career statistics, and performance records on Track Wicket.",
        keywords:
          "cricket player, player profile, cricket statistics, Track Wicket",
    ...
    const title = `${playerName} - ${country} ${role} cricket player profile. View complete career statistics, batting and bowling records`;
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/players/profile/${playerId}`,
      );
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8">
          <div className="glass-card p-8 rounded-2xl text-center max-w-md">
            ...
            <button onClick={handleBack} className="btn-futuristic">
              Back to Search
            </button>
          </div>
        </div>
  ...
  const StatTable = memo(({ title, stats, icon: Icon }) => {
    const formats = Object.keys(stats || {}).filter(
      (f) => Object.keys(stats[f] || {}).length > 0,
    );
    if (formats.length === 0)
      return (
    ...
    return (
      <div className="glass-card rounded-2xl overflow-hidden mb-6">
        ...
        >
          Track Wicket - {displayName} Cricket Player Profile
        </h1>
            ...
            <thead className="bg-secondary/30">
              <tr>
                ...
                {formats.map((f) => (
                  <th
                    key={f}
                    className="px-4 py-3 text-right text-xs font-bold text-primary uppercase tracking-wider min-w-[80px]"
                  >
                    {f}
                  </th>
                ))}
  ...
  const RankingView = () => {
    ...
    if (formats.length === 0) {
      return (
        <div className="text-center p-8 glass-card rounded-2xl">
          <p className="text-lg font-semibold text-muted-foreground">
            ICC Rankings data not available.
          </p>
        </div>
      );
    }
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {formats.map((format) => (
            <div
              key={format}
              className="p-5 bg-gradient-to-br from-primary/10 to-accent/10 rounded-xl border border-primary/20 shadow-cyber"
            >
              ...
              <p className="text-xs text-muted-foreground">
                Best: #{currentRanks[format]?.best_rank || "N/A"}
              </p>
            </div>
          ))}
        </div>
  ...
  const StatsView = () => (
    <div className="space-y-6">
      <StatTable
        ...
        stats={batting_career_summary}
        icon={Zap}
      />
      <StatTable
        title="Bowling Career Summary"
        stats={bowling_career_summary}
        icon={Trophy}
      />
    </div>
  );
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {formats.map((format) => (
            <div
              key={format}
              className="p-5 bg-gradient-to-br from-secondary/30 to-secondary/10 rounded-xl border border-border"
            >
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <p className="text-sm font-semibold text-primary">
                    {career_information[format]?.debut || "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Last Played</p>
                  <p className="text-sm font-semibold text-primary">
                    {career_information[format]?.last_played || "N/A"}
                  </p>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 py-8">
          ...
          {/* Tabs */}
          <div
            className="flex flex-wrap gap-2 mb-8 p-2 glass-card rounded-xl justify-center sm:justify-start animate-fade-in-up"
            style={{ animationDelay: "0.1s" }}
          >
            <TabButton tabId="stats" icon={Zap}>
              Career Stats
            </TabButton>
            <TabButton tabId="ranking" icon={ListOrdered}>
              ICC Rankings
            </TabButton>
            <TabButton tabId="career" icon={Calendar}>
              Career Info
            </TabButton>
          </div>
