...
const ScorecardView = memo(({ scorecardData, liveMiniscore }) => {
  ...
  const displayInnings = inningsList.find(
    (i) => i.inningsId === activeInningsCard,
  );
  ...
  const normalizePartnershipsData = (partnershipsData) => {
    ...
    return Object.values(partnershipsData).filter(
      (p) => p.totalRuns !== undefined,
    );
  };
              ...
              {batsmen.map((b) => (
                ...
                >
                  <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-foreground flex items-center gap-2">
                    ...
                    {liveMiniscore?.batsmanStriker?.batName === b.batName && (
                      <span className="text-primary text-base animate-bounce">
                        🏏
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-xs text-muted-foreground italic">
                    {/* Updated Logic: Show DND if balls are 0 and there is no dismissal description */}
                    {b.balls === 0 && (b.outDesc === "" || !b.outDesc)
                      ? "DNB"
                      : b.outDesc || "not out"}
                  </td>
            ...
            <div className="flex flex-col gap-2">
              <p className="text-sm font-bold text-foreground">
                ...
                <span className="text-xs text-muted-foreground ml-2">
                  (WD: {extrasData.wides}, NB: {extrasData.noBalls}, LB:{" "}
                  {extrasData.legByes}, B: {extrasData.byes})
                </span>
              </p>
            </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {wicket.wktRuns}/{wicket.wktNbr} in {wicket.wktOver}{" "}
                          overs
                        </p>
                      </div>
                    </div>
                  ...
                  >
                    <div className="flex-1">
                      <p className="text-sm font-bold text-foreground mb-1">
                        {partnership.bat1Name || "N/A"} &{" "}
                        {partnership.bat2Name || "N/A"}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {partnership.bat1Runs || 0}({partnership.bat1Balls || 0}
          ...
          <tbody className="divide-y divide-border/30 bg-card/80">
            {bowlers.map((b) => (
              ...
              >
                <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-foreground flex items-center gap-2">
                  {b.bowlName}
                  {liveMiniscore?.bowlerStriker?.bowlName === b.bowlName && (
          ...
          <div className="border border-border rounded-xl overflow-hidden">
            <h3 className="text-xl font-bold text-foreground p-4 bg-gradient-to-r from-secondary/30 to-secondary/10 flex justify-between items-center border-b border-border">
              ...
              <span className="text-base font-normal text-muted-foreground">
                {displayInnings.scoreDetails.runs}/
                {displayInnings.scoreDetails.wickets} (
                {displayInnings.scoreDetails.overs} Ov)
              </span>
            </h3>
            <BattingTable
...
const ScorecardPage = ({ theme, toggleTheme }) => {
  // Accepting theme props
  ...
  const fetchAllData = useCallback(async () => {
    try {
      const [detailsResponse, scorecardResponse] = await Promise.all([
        ...
        fetch(
          `${process.env.REACT_APP_API_BASE_URL}/match-scorecard/${matchId}`,
        ),
      ]);
      ...
      if (detailsResult.status === "success" && isMountedRef.current) {
        ...
        if (
          JSON.stringify(prevDataRef.current.live) !==
      ...
      if (
        scorecardResult.status === "success" &&
        scorecardResult.data &&
        isMountedRef.current
      ) {
        const newScorecardData = scorecardResult.data;
        if (
          JSON.stringify(prevDataRef.current.scorecard) !==
          JSON.stringify(newScorecardData)
        ) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <Helmet>
        ...
        <meta
          name="keywords"
          content={`${team1Name} vs ${team2Name} scorecard, ${seriesName} live, cricket stats, match result, batting card`}
        />
              ...
              <div className="flex items-start gap-4 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span
                      className={`px-3 py-1.5 rounded-full text-xs font-bold shadow-lg ${getFormatBadgeColor(matchHeader.matchFormat)}`}
                    >
